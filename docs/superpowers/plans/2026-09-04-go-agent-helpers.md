...
- PR titles and bodies need Rei's approval before a PR is opened. **One PR for the whole feature in `arcjet-go`**, not one per phase: the plan and spec live on the branch until Task 25 deletes them, and `arcjet-go` is public, so a per-phase PR would publish them. Foreign repositories keep their own PRs, since they carry no planning files. Commit messages end with `Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>`.
